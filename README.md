...
| 检索增强与长文本上下文 | 结合 BM25+Dense、Contextual BM25、RRF、Rerank 和 Small-to-Big，让模型回答时可以使用可追溯的教材片段和父段落上下文。 |
| Workflow 可信回答控制 | 将问题分类、会话记忆、Query Rewrite、检索策略选择、v2 证据 Guard、回答生成和拒答控制拆成可观测节点。 |
...
bge-m3、BM25+Dense、Contextual BM25、RRF、Rerank、Small-to-Big、Workflow、FastAPI、Streamlit
```
...
- 已支持 `vector`、`dense_rerank`、`bm25_hybrid`、`contextual_hybrid` 四种检索模式，以及 Small-to-Big 长文本上下文扩展。
- 已实现 LangGraph Workflow 和本地状态机 fallback，支持会话记忆、Query Rewrite、v2 上下文证据判断、来源引用和无资料拒答。
...
端到端 Workflow guard smoke test 覆盖 `/rag_chat`、LangGraph Workflow 和本地 Workflow 三条路径，共 24 条样例，并校验支持题是否命中预期来源。当前 v2 Guard smoke mock 回归通过率为 100%，错误数为 0，报告见 `eval_results/rag_workflow_guard/smoke_mock_guard_v2_source_mismatch_final/`。扩展回归集覆盖 76 个唯一样例、228 条路径结果，当前 mock 总通过率为 100%，guard 拒答通过率为 100%，支持题来源命中率为 100%，错误数为 0。扩展评测同时保留 `primary_expected_source` 诊断口径，主来源命中率为 96.55%，其余 6 条为合理来源别名命中。详细报告见 `eval_results/rag_workflow_guard/extended_mock_guard_v2_source_mismatch_final/`。

真实模型链路已在 Vast RTX 5060 Ti 上通过 `local_sft` 后端验证。v2.1 QLoRA adapter 的 smoke 回归 24/24 通过，扩展回归使用 `--no-rerank` 跑完 228/228，overall、guard 拒答和支持题来源命中率均为 100%，错误数为 0。报告见 `eval_results/rag_workflow_guard/local_sft_smoke_v21_vast/` 和 `eval_results/rag_workflow_guard/local_sft_extended_v21_vast_64tok_no_rerank/`。
...
| `POST /rag_chat` | 检索增强问答接口，返回回答、来源、检索片段、support_level、evidence_score 和上下文充分性判断。 |
...
├── local_sft_server.py            # OpenAI-compatible QLoRA adapter 推理服务
├── build_kb.py                    # 知识库构建入口
...
- [docs/experiment_results.md](docs/experiment_results.md)：SFT、mock guard 和真实 local_sft 的最终实验结果总览。
